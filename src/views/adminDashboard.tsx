...
export default function AdminDashboard(props: Props) {
  ...
  const [currentTab, setCurrentTab] = React.useState("franchises");
  ...
  React.useEffect(() => {
    (async () => {
      const result = await pizzaService.listUsers(
        userPage,
        usersPerPage,
        userFilterRef.current?.value || "*"
      );
      setUserList(result.users);
      setMoreUsers(result.more);
    })();
  }, [props.user, userPage]);
          ...
          <div className="flex flex-row justify-start gap-4 items-baseline">
            <h3
              className={`text-xl cursor-pointer ${
                currentTab === "franchises"
                  ? "text-orange-400 font-bold"
                  : "text-neutral-100"
              }`}
              onClick={() => setCurrentTab("franchises")}
            >
              Franchises
            </h3>
            <h3
                              ...
                              <td className="text-start px-2 whitespace-nowrap text-sm font-normal text-gray-800">
                                {(() => {
                                  ...
                                  if (roles.includes(Role.Admin))
                                    return "admin";
                                  if (roles.includes(Role.Franchisee))
                                    return "franchisee";
                                  if (roles.includes(Role.Diner))
                                    return "diner";
                                  return "User";
                                })()}
                              </td>
